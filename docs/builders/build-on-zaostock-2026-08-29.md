...
| Set times | **Not public, and not in any API** (Zaal, 2026-09-12: "name the act, not the slot"). The order is public, the clock is not. Each act reads its own time on its own backstage page. If you are building anything that shows a schedule, build it around the order |
