# Message - Zaal to DJ Aquavantes - WITHDRAWN

**WITHDRAWN 27 August, late (Q16): there is no DJ on the day.** Zaal's call. Do
not send this in any form. Steve is told in `msg-steve-peer-2026-08-27.md`.
Kept for the record only.
The body below is STALE against the 27 Aug verdicts (45-minute sets, four
acts, a DJ) and is deliberately not corrected - it must never be sent.

**DO NOT SEND.** Paste-ready once Zaal says yes - but **send this after
Steve answers booked-which, or send it through Steve**, because Aquavantes is
Steve's contact and on our side he is proposed, not booked (Zaal, 27 Aug). The
working doc's own note (24 Aug): confirm he is booked AND knows the job,
because covering changeovers is different from playing an opening set.

- **To:** DJ Aquavantes
- **Contact:** UNSET - via Steve Peer
- **Channel:** UNSET

---

Hi - Zaal from ZAOstock. Steve Peer put your name forward for October 3 in
Ellsworth and I want to make sure we are asking you the right thing, because
it is a specific job rather than a set.

**The job.** The day runs outdoors on the Franklin Street Parklet on one stage,
45-minute sets with 15-minute changeovers, from noon to 4, then WaveWarZ live
battles from 4 to 6. What we need from you is the gaps: music in every
changeover so the street never goes quiet - between 12:45 and 6 - plus the short voting windows inside the battles, and the play-out at
6 as everyone walks next door to Black Moon. Same PA the bands are on, and
you would be checked on it around 11 that morning. Soundcheck the night
before, Friday October 2, is mandatory for everyone on the bill.

**Two questions.**

1. Has Steve actually booked you for this, or is it still a conversation? I
   ask plainly because on our side you are down as proposed, and every gap in
   the day depends on the answer.
2. Is that the job you want - the connective tissue of the day rather than a
   slot of your own? If you would rather have a set as well, say so and we
   will look at it, but the gaps are the thing we need covered.

If it is a yes to both, Steve and I will sort the details with you. Thanks.

Zaal
