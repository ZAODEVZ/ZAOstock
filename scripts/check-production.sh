...
main() {
  ...
  echo "SUMMARY"
  printf '%s' "$SUMMARY"
  echo "  exit=${TOTAL}  (1 preflight failed, 2 backstage failed, 4 phone failed, 8 could not tell)"
  return "$TOTAL"
}
...
selftest() {
  ...
  check() { # expected preflight backstage phone [links]
    ...
    CHECK_PREFLIGHT="exit $p" CHECK_BACKSTAGE="exit $b" CHECK_PHONE="exit $ph" LINKS="$links" QUIET=1 \
      bash "${BASH_SOURCE[0]}" >/dev/null 2>&1
    got=$?
    if [ "$got" = "$want" ]; then echo "  ok    p=$p b=$b ph=$ph links=$(basename "$links") -> $got"
    else echo "  WRONG p=$p b=$b ph=$ph links=$(basename "$links") -> $got, expected $want"; fails=$((fails+1)); fi
  }
