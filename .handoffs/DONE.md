...
- ZAOSTOCK-SITE build-kit (on PR #59, commit on ws/site-round2-0829): /build renders docs/builders/build-on-zaostock-2026-08-29.md for the builder battle (five tracks: organization board, Unlock ticketing, Decentraland with Quan, WaveWarZ parklet front end, POIDH collaboration board; share loop = reach Zaal, Space + stream); llms.txt rewritten from src/content (it carried the $500+ tiers and "lineup once every set is locked"). NEEDS ZAAL: Vercel Production Supabase env points at etwvzrmlxeobinrlytza (cowork), not yjrlaxpjusmrfylumban - /api/events "table public.events not found", /circles 500, lineup API degraded. Four env vars on the Vercel project fix it.
- ZAOSTOCK-SITE audit (PR #59): docs/audit/project-audit-2026-08-29.md - measured project audit + Iman's 28 Aug API audit reviewed line by line, all code-level claims verified. Landed: rider form guarded (was dropping riders), public artist pages confirmed-only and held to 1 Sep, lineup fallback slug, async constant-work login shared by 3 routes, claim_token stripped from the team list, react cache on roster/artist helpers, inactivity 45d, scope enum, events debug fields gone, README, npm audit fix (Next 16.3.3, 0 high, build green), PR #50 closed with its copy salvaged into the press kit, issue #55 root-caused in a comment. Tests 47/47, tsc clean. NEEDS ZAAL in order: rotate team codes; Vercel Supabase env to yjrlaxpjusmrfylumban + redeploy + a second person on that Vercel team; read the artist status CHECK constraint; protect main; one tap to delete 40 merged branches (list in the audit).
- ZAOSTOCK-SITE team-retired (PR #60): Zaal 29 Aug - the team does not run through /team; codes removed. TEAM_DASHBOARD_RETIRED: all /api/team/* 401, six credential routes 410, cron no-op, /team* pages show the pointer to the Organizing Doc, /team/m/* 404, codes script + roster out of the tree, plan exported to docs/plans/team-plan-export-2026-08-29.md, review + stage-two list in docs/site/team-dashboard-review-2026-08-29.md. Workflow proposal (Discord + Organizing Doc + Google Calendar) at ~/zao-vault/notes/team-workflow-2026-08-29.md; roster message on Zaal's clipboard. Tests 47/47, tsc clean. NEEDS ZAAL: send the roster message; confirm channels + calendar; say the word to insert "How we work" and "The plan" tabs into the doc; export CSVs from the real project before stage two deletes tables.
