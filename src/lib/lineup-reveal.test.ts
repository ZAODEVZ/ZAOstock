...
// the 9 September reveal, and the fallback lineup must answer to the slug the
...
describe('lineup reveal gate', () => {
  it('is closed before 9 September 2026 and open from that day', () => {
    ...
    expect(lineupIsPublic(new Date('2026-09-08T23:59:59Z'))).toBe(false);
    // The bug this guards: midnight UTC on the 9th is 8 PM on the 8th in
    // Ellsworth. The gate must still be SHUT.
    expect(lineupIsPublic(new Date('2026-09-09T00:00:00Z'))).toBe(false);
    expect(lineupIsPublic(new Date('2026-09-09T03:59:59Z'))).toBe(false);
    // 04:00Z is midnight in Ellsworth on the 9th. EDT, UTC-4, until 1 November. Open.
    expect(lineupIsPublic(new Date('2026-09-09T04:00:00Z'))).toBe(true);
