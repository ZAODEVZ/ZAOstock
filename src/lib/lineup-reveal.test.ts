...
// the 20 September reveal, and the fallback lineup must answer to the slug the
...
describe('lineup reveal gate', () => {
  it('is closed before 20 September 2026 and open from that day', () => {
    ...
    expect(lineupIsPublic(new Date('2026-09-19T23:59:59Z'))).toBe(false);
    // The bug this guards: midnight UTC on the 20th is 8 PM on the 19th in
    // Ellsworth. The gate must still be SHUT.
    expect(lineupIsPublic(new Date('2026-09-20T00:00:00Z'))).toBe(false);
    expect(lineupIsPublic(new Date('2026-09-20T03:59:59Z'))).toBe(false);
    // 04:00Z is midnight in Ellsworth on the 20th - EDT, UTC-4, and DST does
    // not end until 1 November, so the offset is the same as it was. Open.
    expect(lineupIsPublic(new Date('2026-09-20T04:00:00Z'))).toBe(true);
