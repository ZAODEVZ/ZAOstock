...
// the 8 September reveal, and the fallback lineup must answer to the slug the
...
describe('lineup reveal gate', () => {
  it('is closed before 8 September 2026 and open from that day', () => {
    ...
    // The day this gate used to open on, 7 September. It must now be SHUT, or
    // the move of the reveal did not actually take.
    expect(lineupIsPublic(new Date('2026-09-07T04:00:00Z'))).toBe(false);
    expect(lineupIsPublic(new Date('2026-09-07T23:59:59Z'))).toBe(false);
    // The bug this guards: midnight UTC on the 8th is 8 PM on the 7th in
    // Ellsworth. The gate must still be SHUT.
    expect(lineupIsPublic(new Date('2026-09-08T00:00:00Z'))).toBe(false);
    expect(lineupIsPublic(new Date('2026-09-08T03:59:59Z'))).toBe(false);
    // 04:00Z is midnight in Ellsworth on the 8th. EDT, UTC-4, until 1 November. Open.
    expect(lineupIsPublic(new Date('2026-09-08T04:00:00Z'))).toBe(true);
